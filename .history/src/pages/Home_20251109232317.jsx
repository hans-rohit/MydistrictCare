...
export default function Home({ showIntro = true }) {
  ...
  // Initialize filters from URL on mount (wait for profile to load first)
  useEffect(() => {
    if (!profile) return; // Wait for profile/role to be determined
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [profile]); // Run when profile loads (which determines isSuperAdmin)
