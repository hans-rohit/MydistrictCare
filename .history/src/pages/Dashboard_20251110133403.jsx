...
export default function Dashboard() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      // By Month - Dynamic based on time range
      const monthCounts = {};
      const dayCounts = {};
      const monthNames = [
        "Jan",
      ...
      if (timeRange === "7days") {
        ...
        byMonth = Object.entries(dayCounts)
          .map(([day, count]) => ({ period: day, count }))
          .slice(-7);
      } else if (timeRange === "30days") {
        // Show daily trend for last 30 days grouped by week
        const weekCounts = {};
        posts.forEach((post) => {
          if (post.createdAt?.toDate) {
            const date = post.createdAt.toDate();
        ...
        byMonth = Object.entries(weekCounts)
