...
import {
  ...
  VStack, Progress, Alert, AlertIcon, HStack, Text, Link, useToast, SimpleGrid
...
export default function CreatePost() {
  ...
  useEffect(() => {
    getCurrentPosition()
      ...
      .catch(() => {
        setLocStatus('Could not auto-detect location. Enter manually or use the controls.')
      })
  }, [setCoordinates])
  ...
  const handleMapClick = (mapLat, mapLng) => {
    setCoordinates(mapLat, mapLng, { status: 'Location picked from map', clearLabel: true })
  }

  const handleMarkerDrag = (event) => {
    const marker = event.target
    const { lat: markerLat, lng: markerLng } = marker.getLatLng()
    setCoordinates(markerLat, markerLng, { status: 'Marker dragged', clearLabel: true })
  }
  ...
  const fetchSearchResults = useCallback(async (query) => {
    ...
    try {
      ...
      const response = await fetch(`${NOMINATIM_URL}?${params.toString()}`, {
        headers: {
          'Accept-Language': 'en',
          'User-Agent': 'district-care-app/1.0 (contact@districtcare.example)'
        }
      })
      if (!response.ok) {
      ...
      if (!Array.isArray(results) || results.length === 0) {
        ...
        toast({
          ...
          description: 'Try searching with a more specific address or landmark.',
          status: 'warning',
          duration: 4000,
          isClosable: true,
        })
      ...
      toast({
        title: 'Search failed',
        description: error.message || 'Unable to search for that location right now.',
        status: 'error',
        duration: 5000,
        isClosable: true,
      })
      ...
      <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={8} alignItems="start">
        <Box>
          <Box position="relative" borderWidth="1px" borderRadius="lg" overflow="hidden">
              ...
              <VStack spacing={3} align="stretch">
                <VStack spacing={2} align="stretch" pointerEvents="auto">
                  ...
                  {searchResults.length > 0 && (
                    <Box bg="white" borderRadius="md" shadow="sm" maxH="180px" overflowY="auto">
            ...
            >
              ...
              {hasCoordinates && (
                <MapViewUpdater center={[parsedLat, parsedLng]} zoom={SELECTED_ZOOM} />
              )}
          ...
          <VStack align="stretch" spacing={2} mt={4}>
            ...
            <Text fontSize="sm" color="gray.600">
              {locStatus || 'Use the map tools or drag the marker to update the coordinates.'}
            </Text>
            {hasCoordinates && (
              <Text fontSize="sm" color="gray.600">
        ...
        <Box as="form" onSubmit={handleSubmit}>
          <VStack spacing={4} align="stretch">
            ...
            <Text fontSize="sm" color="gray.600">
              These coordinates update automatically when you interact with the map.
            </Text>
            <Button type="submit" colorScheme="blue" isLoading={submitting} isDisabled={!isFormValid}>
              Submit
            </Button>
          </VStack>
        </Box>
