...
export default function Home({ showIntro = true }) {
  ...
  const filteredPosts = useMemo(() => {
    if (!isSearchingActive || !appliedSearch.trim()) return posts;
    const queryText = appliedSearch.trim().toLowerCase();
    return posts.filter((p) =>
      (p.title || "").toLowerCase().includes(queryText)
    );
  }, [posts, isSearchingActive, appliedSearch]);
  ...
  const fetchSearch = async (keyword, status, dept) => {
    setIsLoading(true);
    try {
      const constraints = [];
      if (dept) constraints.push(whereField("departmentTag", dept));
      if (status && status !== "deleted")
        constraints.push(whereField("status", status));
      // Don't filter by title on server - do it client-side for better flexibility

      // Build Firestore query with dynamic where clauses
      let qRef = collection(db, "posts");
      let q = query(qRef, ...constraints.map((c) => c));
      const snapshot = await getDocs(q);
      let list = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
      ...
      // Apply date range client-side
      const withDate = (() => {
        if (!appliedFrom && !appliedTo) return list;
