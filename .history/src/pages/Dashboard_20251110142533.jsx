        ...
        if (deptPosts.length > 0) {
          const totalDays = deptPosts.reduce((sum, post) => {
            if (post.createdAt?.toDate) {
              ...
              // Use resolvedAt if available, otherwise use current time
              if (post.resolvedAt?.toDate) {
                resolved = post.resolvedAt.toDate();
              } else if (post.resolvedAt instanceof Date) {
                resolved = post.resolvedAt;
              } else {
                // Fallback to current time for old resolved issues
                resolved = new Date();
              }
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={stats.responseTime}>
                ...
                <Bar dataKey="avgDays" name="Avg Days to Resolve" fill="#667eea">
                  {stats.responseTime.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={departmentColors[entry.department] || "#667eea"} 
                    />
                  ))}
                </Bar>
              </BarChart>
